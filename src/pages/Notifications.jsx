...
import { Bell, BellOff, CheckCircle2, Settings, Smartphone, RefreshCw } from 'lucide-react';
  ...
  const handleEnable = async () => {
    ...
    try {
      ...
      if (ok) {
        ...
        toast.error('הגישה להתראות נחסמה', { description: 'יש לאפשר ידנית בהגדרות המכשיר' });
      }
    } finally {
      setLoading(false);
    }
          ...
          >
            ...
            <div className="rounded-xl bg-amber-500/8 ring-1 ring-amber-400/20 px-4 py-3 flex items-center gap-2.5">
              ...
              <p className="text-amber-200/80 text-xs font-bold leading-snug">
                לאחר שינוי ההגדרות, חזור לכאן — הכפתור יופיע אוטומטית
              </p>
            </div>
          </motion.div>
        )}
