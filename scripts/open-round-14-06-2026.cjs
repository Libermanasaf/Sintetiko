/**
 * One-off: open a regular round for 14/06/2026 (same rosters as 10.6).
 * Opening: הצהובים(0) vs הכתומים(2) -> openingTeams:[0,2]. +1 appearance for all
 * 18 players. is_published:false (admin finishes in-app).
 * Team order: 0=הצהובים 1=הכחולים 2=הכתומים.
 *
 *   SUPABASE_SERVICE_ROLE_KEY=... node scripts/open-round-14-06-2026.cjs            (dry run)
 *   SUPABASE_SERVICE_ROLE_KEY=... node scripts/open-round-14-06-2026.cjs --apply
 */
const fs = require('fs');
...
const ROSTERS = {
  'הצהובים (0)': ['דוד דסלין', 'אביחי שרה קאן', 'דור אזורד', 'אריאל רביבו', 'גל בן חמו', 'יניב אזולאי'],
...
async function main() {
  ...
  const roundPayload = {
    ...
    date: '2026-06-14T18:00:00.000+00:00',
    teams,
    goalkeepers: {},
